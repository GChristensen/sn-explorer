...
(defparameter *agent-name-scanner*
  (create-scanner "<title>.*\\|\\s*(.*)</title>"
				  :single-line-mode t))
...
(defclass sn-session-vkontakte (sn-session) 
  ((sn-x-agent-id :reader sn-x-agent-id :initform nil)
   (sn-x-agent-name :reader sn-x-agent-name :initform nil)))
    ...
    (let ((body
	  ...
	  (register-groups-bind (id-string)
		...
		(when (not (null id-string))
		  ...
		  (register-groups-bind (name-string)
			  (*agent-name-scanner* body)
			(setf (slot-value session 'sn-x-agent-name) name-string))))
...
(defmethod sn-agent-name ((session sn-session-vkontakte))
  (sn-x-agent-name session))
