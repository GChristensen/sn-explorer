...
(defparameter +working-root+ ".sn-explorer")
...
(defun get-quit-button-text ()
  (let* ((sn-session (session-value 'sn-session))
		 (agent-name (when sn-session (sn-agent-name sn-session))))
	(if agent-name
		(format nil "~a (~a)"  #$exit 
				(babel:octets-to-string
				 (babel:string-to-octets agent-name :encoding :utf-8)
				 :encoding :iso-8859-1))
		"")))
...
(define-easy-handler (login-handler :uri "/login"
  ...
  (with-useragent-language
	...
	(condition ()
	  (setf (session-value 'sn-session) nil)
